...
function runArcheTokens(text) {
    return new Promise((resolve, reject) => {
        const child = (0, node_child_process_1.spawn)(tokensPath, [], { stdio: ["pipe", "pipe", "pipe"] });
        let stdout = "";
        let stderr = "";
...
connection.languages.semanticTokens.on(async (params) => {
    ...
    try {
        ...
        if (!warnedMissingBinary) {
            ...
            connection.window.showWarningMessage(`arche: could not run '${tokensPath}'. Build it from the arche repo (make arche-tokens) ` +
                `and put it on PATH or set initializationOptions.tokensPath. (${err.message})`);
        }
        return builder.build();
    }
    /*
