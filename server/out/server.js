...
var Analyzer = class {
  constructor(cmd) {
    ...
    try {
      this.proc = (0, import_node_child_process.spawn)(this.cmd, ["--serve"], { stdio: ["pipe", "pipe", "pipe"] });
    } catch (err) {
  ...
  warnMissing(msg) {
    ...
    connection.window.showWarningMessage(
      `arche: could not run '${this.cmd}'. Build it from the arche repo (make build/arche-analyzer) and put it on PATH or set initializationOptions.analyzerPath. (${msg})`
    );
  }
  onData(chunk) {
    this.buf += chunk;
...
connection.languages.inlayHint.on(async (params) => {
  ...
  for (const line of await analyzer.hints(uriToPath(params.textDocument.uri))) {
    ...
    hints.push({
      ...
      label,
      kind: kind === "param" ? import_node.InlayHintKind.Parameter : import_node.InlayHintKind.Type,
      paddingLeft: padL,
      paddingRight: padR
    });
  }
